  ...
  return (
    <ErrorBoundary label="Settings">
      <>
        ...
        <div style={{ position: "fixed", top: 56, right: 12, zIndex: 61, background: "#fff", borderRadius: 16, border: "1px solid var(--bd)", boxShadow: "0 8px 30px rgba(0,0,0,.12)", padding: 8, minWidth: 200 }}>
          ...
          <button onClick={() => { onClose(); setShowSupport(true) }} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--tx)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F4AC}"} Contact Support</button>
          <button onClick={() => setShowChangePw(true)} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--tx)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F511}"} Change Password</button>
          {profile?.subscription_status === "active" && <button onClick={() => { onClose(); openPortal() }} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--tx)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F4B3}"} Manage Subscription</button>}
          ...
          : profile?.subscription_status !== "active" && <button onClick={() => { onClose(); startCheckout("standard") }} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--acc)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u2B50"} Subscribe {"\u2014"} {"\u20AC"}7.99/mo</button>}
          <button onClick={() => { onClose(); setShowFamily(true) }} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--tx)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F468}\u200D\u{1F469}\u200D\u{1F467}\u200D\u{1F466}"} Family Members</button>
          <a href="/privacy" style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--tx)", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8, textDecoration: "none" }}>{"\u{1F512}"} Privacy & Data</a>
          ...
          <div style={{ borderTop: "1px solid var(--bd)", marginTop: 4, paddingTop: 4 }}>
            <button onClick={() => setShowDeleteAcct(true)} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#dc2626", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F5D1}\uFE0F"} Delete Account</button>
            <button onClick={() => { onClose(); onLogout() }} style={{ width: "100%", padding: "10px 14px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#dc2626", fontFamily: "var(--sn)", textAlign: "left", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>{"\u{1F6AA}"} Log out</button>
          </div>
        </div>
        ...
        {showFamily && <div className="mbg fi" onClick={() => setShowFamily(false)}>
          <div className="mbox" onClick={e => e.stopPropagation()} style={{ maxHeight: "80vh" }}>
            ...
            <div>
              ...
              {familyMembers.filter(fm => fm.id !== user.id).map(fm => { const rl = fm.family_role || "admin"; const roleBadge = { admin: "\u{1F468}\u200D\u{1F469}\u200D\u{1F467} Parent", carer: "\u{1F9D1}\u200D\u{1F373} Carer", viewer: "\u{1F441}\uFE0F Viewer" }; return <div key={fm.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid var(--bd)" }}>
