...
export default function ThingsToDoSection({allLocs,kids,userLoc,setUserLoc,userId,onEventAdded}){
  ...
  if(loadingT)return <p style={{textAlign:"center",padding:20,color:"var(--color-muted)"}}>Finding things to do near you...</p>;
  if(things.length===0)return <div style={{padding:20,borderRadius:14,border:"2px dashed var(--color-border)",textAlign:"center"}}><div style={{fontSize:28,marginBottom:6}}>🔍</div><p style={{fontSize:13,color:"var(--color-muted)"}}>No activities found in your area yet</p><p style={{fontSize:12,color:"var(--color-muted)",marginTop:4}}>We are building our database. Check back soon.</p></div>;
  ...
  return <div>
    <h3 style={{fontFamily:"var(--font-serif)",fontSize:17,fontWeight:700,color:"var(--color-primary)",marginBottom:4}}>Things to do near you</h3>
    <p style={{fontSize:12,color:"var(--color-muted)",marginBottom:10}}>{allLocs.length>0?"Sorted by distance from your locations":"Add locations for distance-sorted results"}</p>
    <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:12,WebkitOverflowScrolling:"touch"}}>
      <button onClick={()=>setTdFilter("all")} className={"pill "+(tdFilter==="all"?"pill-active":"pill-inactive")}>All ({things.length})</button>
      {hasKids&&<button onClick={()=>setAgeFilter(!ageFilter)} className={"pill "+(ageFilter?"pill-active":"pill-inactive")}>👧 My kids' ages</button>}
      {sortedCats.map(([cat,cnt])=><button key={cat} onClick={()=>setTdFilter(tdFilter===cat?"all":cat)} className={"pill "+(tdFilter===cat?"pill-active":"pill-inactive")}>{catIcon[cat]||"🎯"} {catLabel[cat]||cat} ({cnt})</button>)}
    </div>
    {filtered.map(t=>{
      ...
      return <div key={t.id} style={{background:"var(--color-card)",border:"1px solid var(--color-border)",borderRadius:16,padding:16,marginBottom:10,boxShadow:"var(--shadow)"}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
          <div style={{width:40,height:40,borderRadius:10,background:"var(--color-sage)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{catIcon[t.category]||"🎯"}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:14,fontWeight:700,color:"var(--color-text)"}}>{t.title}</div>
            {t.location_name&&<div style={{fontSize:12,color:"var(--color-muted)",marginTop:2}}>{t.location_name}</div>}
            {t.description&&<div style={{fontSize:12,color:"var(--color-muted)",marginTop:4,lineHeight:1.4}}>{t.description.length>120?t.description.substring(0,120)+"...":t.description}</div>}
            <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>
              {dist!==null&&<span style={{fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:6,background:"#e8f0f5",color:"var(--color-primary-light)"}}>📍 {fmtDist(dist)}</span>}
              {t.cost_eur!==null&&<span style={{fontSize:11,fontWeight:700,padding:"3px 8px",borderRadius:6,background:"var(--color-accent-bg)",color:"#c44030"}}>{Number(t.cost_eur)>0?"€"+t.cost_eur:"Free"}</span>}
              ...
              {t.age_max<99&&t.age_max<=14&&<span style={{fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:6,background:"#f0f0f0",color:"var(--color-muted)"}}>Ages {t.age_min}–{t.age_max}</span>}
        ...
        <div style={{display:"flex",gap:8,marginTop:12}}>
          ...
          }} style={{flex:1,textAlign:"center",padding:12,borderRadius:14,border:"1.5px solid var(--color-primary)",background:"none",color:"var(--color-primary)",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"var(--font-sans)",minHeight:44}}>📅 Add to schedule</button>
          :<span style={{flex:1,textAlign:"center",padding:12,borderRadius:14,background:"var(--color-sage)",color:"var(--color-primary-light)",fontSize:13,fontWeight:700,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>✓ Added</span>}
          {t.website_url&&<a href={t.website_url} target="_blank" rel="noopener" style={{flex:1,textAlign:"center",padding:12,borderRadius:14,background:"var(--color-primary)",color:"#fff",fontSize:13,fontWeight:700,textDecoration:"none",minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}>Website →</a>}
        </div>
      </div>;
    })}
    {filtered.length===0&&<p style={{textAlign:"center",padding:20,color:"var(--color-muted)"}}>No activities match this filter</p>}
