...
export default function DiscoverResults({query,onAdd,userLoc}){
  ...
  return <div style={{marginBottom:24}}>
    <h3 style={{fontFamily:"var(--font-serif)",fontSize:17,fontWeight:700,color:"var(--color-primary)",marginBottom:4}}>{catName[query]||query}</h3>
    <p style={{fontSize:12,color:"var(--color-muted)",marginBottom:12}}>{filtered.length} club{filtered.length!==1?"s":""} near you</p>
    {loading&&<p style={{textAlign:"center",padding:20,color:"var(--color-muted)"}}>Loading...</p>}
    {!loading&&filtered.length===0&&<p style={{textAlign:"center",padding:20,color:"var(--color-muted)"}}>No clubs found near you for "{query}"</p>}
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
    {filtered.map(club=>{const dist=(()=>{if(!club.latitude)return null;const cL=Number(club.latitude),cN=Number(club.longitude);let min=999;if(userLoc)min=Math.min(min,calcKm(userLoc.lat,userLoc.lng,cL,cN));return min<999?min:null})();return <div key={club.id} style={{background:"var(--color-card)",border:"1px solid var(--color-border)",borderRadius:14,padding:14,boxShadow:"var(--shadow)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1}}>
          ...
          {club.website_url&&<a href={club.website_url} target="_blank" rel="noopener" onClick={e=>e.stopPropagation()} style={{fontSize:11,color:"var(--color-accent)",textDecoration:"none"}}>🌐 {club.website_url.replace("https://","").replace("www.","").replace(/\/$/,"")}</a>}
        </div>
        <button onClick={()=>onAdd(club)} style={{padding:"6px 14px",borderRadius:10,border:"1.5px solid var(--color-primary)",background:"none",fontSize:12,fontWeight:700,color:"var(--color-primary)",cursor:"pointer",fontFamily:"var(--font-sans)",whiteSpace:"nowrap"}}>+ Add</button>
      </div>
    </div>})}
    </div>
  </div>;
