...
export default function EventDetailModal({event,open,onClose,onDelete,onDriverChange,onAttendeesChange,onMarkPaid,onColourChange,adults,familyAll}){
  ...
  if(isPayment){
    ...
    return <div onClick={onClose} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(10,15,20,.45)",backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} className="modal-overlay">
      <div onClick={e=>e.stopPropagation()} className="modal-sheet" style={{background:"var(--color-card)",borderRadius:20,padding:24,width:"100%",maxWidth:400,boxShadow:"var(--shadow-xl)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:isOverdue?"#dc2626":isDueSoon?"var(--color-accent)":"#c4960c",textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>
              {isOverdue?"Overdue":isDueSoon?"Due soon":"Payment due"}
            </div>
            <h3 style={{fontFamily:"var(--font-serif)",fontSize:20,fontWeight:800,color:"var(--color-primary)",lineHeight:1.2}}>{event.payDescription||"Payment"}</h3>
          </div>
          <button onClick={onClose} style={{width:32,height:32,borderRadius:10,border:"none",background:"var(--color-warm)",cursor:"pointer",fontSize:16,color:"var(--color-muted)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
        </div>
        ...
        {/* Details */}
        <div style={{borderTop:"1px solid var(--color-border)",borderBottom:"1px solid var(--color-border)",padding:"8px 0",marginBottom:20}}>
  ...
  return <div onClick={onClose} style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(10,15,20,.45)",backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} className="modal-overlay">
    <div onClick={e=>e.stopPropagation()} className="modal-sheet" style={{background:"var(--color-card)",borderRadius:20,padding:24,width:"100%",maxWidth:400,boxShadow:"var(--shadow-xl)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <h3 style={{fontFamily:"var(--font-serif)",fontSize:20,fontWeight:800,color:"var(--color-primary)",lineHeight:1.2,maxWidth:"85%"}}>{event.member} — {event.club||event.title||"Event"}</h3>
        <button onClick={onClose} style={{width:32,height:32,borderRadius:10,border:"none",background:"var(--color-warm)",cursor:"pointer",fontSize:16,color:"var(--color-muted)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
      </div>
